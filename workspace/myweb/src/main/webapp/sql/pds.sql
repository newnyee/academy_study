...
INSERT INTO tb_pds (pdsno, wname, subject, passwd, regdate, filename, filesize)
...
FROM (SELECT a.*, ROWNUM AS rnum
      FROM (SELECT pdsno, wname, subject, filename, filesize, regdate, readcnt
            FROM tb_pds
            ORDER BY regdate) a)
ORDER BY rnum DESC;
...
-- 상세조회
SELECT wname, subject, readcnt, regdate, filename, filesize
FROM tb_pds
WHERE pdsno = ?;
