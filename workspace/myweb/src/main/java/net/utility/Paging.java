...
public class Paging {  
  ...
  public String paging1(int totalRecord, int nowPage, int recordPerPage, String col, String word, String filenm){
    ...
    //str.append("현재 페이지: " + nowPage + " / " + totalPage + "&nbsp;&nbsp;");
      ...
      if (nowGrp >= 2){
        str.append("<li><a href='./"+filenm+"?col="+col+"&word="+word+"&nowPage="+_nowPage+"'>&lt;</a></li>");
      } else {
      ...
      for(int i=startPage; i<=endPage; i++){
          ...
          if (nowPage == i){ // 현재 페이지이면 강조 효과
              str.append("<li class='active'><a href='./" + filenm + "?col=" + col + "&word=" + word + "&nowPage=" + i + "'>" + i + "</a></li>");
          }else{
  ...
  public String paging3(int totalRecord, int nowPage, int recordPerPage, String col, String word, String filenm){
    ...
    str.append("  }");
    str.append("</style>");
    str.append("<ul id='paging' class=\"pagination\">");
    //str.append("현재 페이지: " + nowPage + " / " + totalPage + "&nbsp;&nbsp;");

    int _nowPage = (nowGrp-1) * pagePerBlock; // 10개 이전 페이지로 이동
    if (nowGrp >= 2){
      str.append("<li><a href='./"+filenm+"?col="+col+"&word="+word+"&nowPage="+_nowPage+"'><span class='span_box_1'>이전</span></A></li>");
    }
    ...
    for(int i=startPage; i<=endPage; i++){
      ...
      if (nowPage == i){
        ...
        str.append("<li><A href='./"+filenm+"?col="+col+"&word="+word+"&nowPage="+i+"'><span class='span_box_1'>&nbsp;"+i+"&nbsp;</span></A></li>");
      }
    }
    
    _nowPage = (nowGrp * pagePerBlock)+1; // 10개 다음 페이지로 이동
    if (nowGrp < totalGrp){
      str.append("</li><A href='./"+filenm+"?col="+col+"&word="+word+"&nowPage="+_nowPage+"'><span class='span_box_1'>다음</span></A></li>");
    }
    str.append("</ul>");
