...
<%
    ...
    request.setAttribute("eng", 200); //전역변수(부모, 자식 페이지에서만 유효)
    session.setAttribute("mat", 300); //전역변수(모든 페이지에서 유효 + 개인사용자 + 시간)
    application.setAttribute("aver", 200); //전역변수(모든 페이지에서 유효 + 모든사용자가 공유)
%>
    1) JSP<br>
...
%>
    ...
    num : ${num} <%--500출력(pageContext)--%>
    <hr>

    <%-- 각 Scope의 내장변수가 없는 경우 --%>
    JSP : <%=request.getAttribute("uname")%> <%--null출력--%>
    <br>
    EL : ${requestScope.uname} <%--빈문자열 출력--%>
